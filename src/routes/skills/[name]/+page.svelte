<script lang="ts">
	...
	import { ApiError, getSkill, deleteSkill } from '$lib/api/client';
	...
	async function load(n: string) {
		...
		state = { kind: 'loading' };
		try {
			state = { kind: 'ok', item: await getSkill(n) };
		} catch (err) {
	...
	// --- Solo skill user-pack: rimovibili (no editing locale, le skill si importano) ---
	$: deletable = state.kind === 'ok' && state.item.pack === 'user-pack';
	let deleting = false;
	let actionError = '';

	async function removeSkill() {
		if (!confirm(`Eliminare la skill "${name}"? L'azione non è reversibile.`)) return;
		deleting = true;
		actionError = '';
		try {
...
<header class="head">
	...
	<div class="head-actions">
		{#if deletable}
			<button type="button" class="danger" on:click={removeSkill} disabled={deleting}>
...
{#if actionError}<div class="form-error standalone">{actionError}</div>{/if}
...
{:else if state.kind === 'error'}
	<section class="status error">
		<strong>{state.status === 404 ? 'Skill not found.' : `Failed to load skill${state.status ? ` (HTTP ${state.status})` : ''}.`}</strong>
